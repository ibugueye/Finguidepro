...
if 'current_step' not in st.session_state:
    st.session_state.current_step = 0  # Garder comme int pour selectbox
...
def calculate_ratios(balance_sheet, income_statement):
    ...
    try:
        ...
        if passif_circulant > 0:
            ratios['Fond de Roulement'] = actif_circulant - passif_circulant
            ratios['Ratio de Liquidité'] = actif_circulant / passif_circulant
        else:
            ratios['Fond de Roulement'] = actif_circulant
            ratios['Ratio de Liquidité'] = float('inf')
            
        if 'CA' in income_statement:
            ratios['ROA'] = (income_statement.get('Résultat Net', 0.0) / total_actif) * 100 if total_actif > 0 else 0.0
...
def main():
    ...
    with st.sidebar:
        ...
        if st.button("🔁 Réinitialiser les données"):
            ...
            st.session_state.income_statement = {}
            st.session_state.current_step = 0  # Réinitialiser aussi l'étape
            st.rerun()
...
def show_balance_sheet():
    ...
    # Afficher la progression (convertir en float pour la barre de progression)
    ...
    with col1:
        ...
        with st.form("actif_immobilise_form"):
            ...
            if st.form_submit_button("💾 Enregistrer Actif Immobilisé"):
                st.success("Actif immobilisé enregistré!")
        
        # Actif Circulant
        st.markdown("#### Actif Circulant")
        with st.form("actif_circulant_form"):
            ...
            if st.form_submit_button("💾 Enregistrer Actif Circulant"):
                st.success("Actif circulant enregistré!")
    ...
    with col2:
        ...
        with st.form("capitaux_propres_form"):
            ...
            if st.form_submit_button("💾 Enregistrer Capitaux Propres"):
                st.success("Capitaux propres enregistrés!")
        
        # Dettes
        st.markdown("#### Dettes")
        with st.form("dettes_form"):
            ...
            if st.form_submit_button("💾 Enregistrer Dettes"):
                st.success("Dettes enregistrées!")
...
def show_income_statement():
    ...
    with col1:
        ...
        with st.form("produits_form"):
            st.session_state.income_statement['Chiffre d\'affaires'] = st.number_input(
                "Chiffre d'affaires HT:",
                value=float(st.session_state.income_statement['Chiffre d\'affaires']),
                step=1000.0,
                format="%.2f"
            )
            
            st.session_state.income_statement['Produits financiers'] = st.number_input(
                "Produits financiers:",
                value=float(st.session_state.income_statement['Produits financiers']),
                step=1000.0,
                format="%.2f"
            )
            
            if st.form_submit_button("💾 Enregistrer Produits"):
                st.success("Produits enregistrés!")
    ...
    with col2:
        ...
        with st.form("charges_form"):
            ...
            with cols[0]:
                st.session_state.income_statement['Achats de marchandises'] = st.number_input(
                    "Achats marchandises:",
                    value=float(st.session_state.income_statement['Achats de marchandises']),
                    step=1000.0,
                    format="%.2f"
                )
                
                st.session_state.income_statement['Autres achats et charges externes'] = st.number_input(
                    "Autres charges externes:",
                    value=float(st.session_state.income_statement['Autres achats et charges externes']),
                    step=1000.0,
                    format="%.2f"
                )
                
                st.session_state.income_statement['Charges de personnel'] = st.number_input(
                    "Charges de personnel:",
                    value=float(st.session_state.income_statement['Charges de personnel']),
                    step=1000.0,
                    format="%.2f"
                )
            
            with cols[1]:
                st.session_state.income_statement['Variation de stocks'] = st.number_input(
                    "Variation de stocks:",
                    value=float(st.session_state.income_statement['Variation de stocks']),
                    step=1000.0,
                    format="%.2f"
                )
                
                st.session_state.income_statement['Dotations aux amortissements'] = st.number_input(
                    "Dotations amortissements:",
                    value=float(st.session_state.income_statement['Dotations aux amortissements']),
                    step=1000.0,
                    format="%.2f"
                )
                
                st.session_state.income_statement['Charges financières'] = st.number_input(
                    "Charges financières:",
                    value=float(st.session_state.income_statement['Charges financières']),
                    step=1000.0,
                    format="%.2f"
                )
            
            # Impôt sur les bénéfices
            st.session_state.income_statement['Impôt sur les bénéfices'] = st.number_input(
                "Impôt sur les bénéfices:",
                value=float(st.session_state.income_statement['Impôt sur les bénéfices']),
                step=1000.0,
                format="%.2f"
            )
            
            if st.form_submit_button("💾 Enregistrer Charges"):
    ...
    # Calculs
    CA = st.session_state.income_statement['Chiffre d\'affaires']
    achats = st.session_state.income_statement['Achats de marchandises']
    var_stocks = st.session_state.income_statement['Variation de stocks']
    autres_charges = st.session_state.income_statement['Autres achats et charges externes']
    charges_personnel = st.session_state.income_statement['Charges de personnel']
    dotations = st.session_state.income_statement['Dotations aux amortissements']
    produits_financiers = st.session_state.income_statement['Produits financiers']
    charges_financieres = st.session_state.income_statement['Charges financières']
    impot = st.session_state.income_statement['Impôt sur les bénéfices']
    
    marge_commerciale = CA - achats + var_stocks
...
def show_financial_analysis():
    ...
    with tabs[0]:
        ...
        with col1:
            ...
            fr = ratios.get('Fond de Roulement', 0.0)
            liquidite = ratios.get('Ratio de Liquidité', 0.0)
        ...
        with col3:
            ...
            CA = st.session_state.income_statement.get('Chiffre d\'affaires', 1.0)
            
            # Calcul simplifié du résultat net
            charges_totales = sum([v for k, v in st.session_state.income_statement.items() 
                                  if k not in ['Chiffre d\'affaires', 'Produits financiers']])
    ...
    with tabs[3]:
        ...
        total_actif = sum([sum(v.values()) for v in st.session_state.balance_sheet['Actif'].values()])
        ca = st.session_state.income_statement.get('Chiffre d\'affaires', 0.0)
